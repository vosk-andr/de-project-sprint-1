...
-- добавление данных из external_source.craft_products_orders и external_source.customers
UNION
SELECT  t1.order_id,
        t1.order_created_date,
        t1.order_completion_date,
  ...
  FROM external_source.craft_products_orders t1
    JOIN external_source.customers t2 USING(customer_id);
...
-- делаем выборку покупателей, по которым были изменения в DWH. По этим мастерам данные в витрине нужно будет обновить
dwh_update_delta AS ( 
    SELECT     
...
-- делаем расчёт витрины по новым данным. Этой информации по мастерам в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
dwh_delta_insert_result AS ( 
    SELECT  
            ...
            FROM (
-- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров и id самого популярного масера у покупателя
                SELECT     
                        *,
                        RANK() OVER(PARTITION BY T2.customer_id ORDER BY T3.count_product_for_product_type, T3.count_product_for_craftsman_id DESC) AS rank_count_product 
                        FROM ( 
-- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбцов с самой популярной категорией товаров и id самого популярного мастера у покпателя. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                            SELECT 
                                ...
                                INNER JOIN 
-- Эта выборка поможет определить самый популярный товар у покупателя. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самый популярны товар у покупателя. Эта выборка разбита на два запроса (Т31 - для самого популярного товара и Т32 - для самого популярного мастера) по скольку самый популярный товар не всегда продает самый поопулярный мастер и наоборот.
                                    ((SELECT     
                                            dd.customer_id AS customer_id_for_top, 
                                            dd.product_type,
                                            COUNT(dd.product_id) AS count_product_for_product_type
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.product_type
                                                    ORDER BY count_product_for_product_type DESC) as T31
                                 INNER JOIN 
-- Эта выборка поможет определить id самого популярного мастера у покупателя. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет id самого популярного мастера у покупателя. Эта выборка разбита на два запроса (Т31 - для самого популярного товара и Т32 - для самого популярного мастера) по скольку самый популярный товар не всегда продает самый поопулярный мастер и наоборот.
                                    (SELECT     
                                            dd.customer_id AS customer_id_for_top, 
                                            dd.craftsman_id,
                                            COUNT(dd.product_id) AS count_product_for_craftsman_id
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id
                                                    ORDER BY count_product_for_craftsman_id DESC) AS T32 using(customer_id_for_top))                   
                AS T3 ON T2.customer_id = T3.customer_id_for_top                                     
                ) AS T4 WHERE T4.rank_count_product = 1 ORDER BY report_period -- условие помогает оставить в выборке первую строку с наибольшим количеством товара одного типа купленным покупателем и с id мастера который продал наибольшее количество товаров покпателю
),
-- делаем перерасчёт для существующих записей витрины, так как данные обновились за отчётные периоды. Логика похожа на insert, но нужно достать конкретные данные из DWH
dwh_delta_update_result AS ( 
    SELECT 
            ...
            T4.report_period AS report_period
            FROM (
-- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров и id самого популярного мастера у покупателя
                SELECT
                        *,
                        RANK() OVER(PARTITION BY T2.customer_id ORDER BY T3.count_product_for_product_type, T3.count_product_for_craftsman_id DESC) AS rank_count_product 
                        FROM (
-- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбцов с самой популярной категорией товаров и id самого популярного мастера у покпателя. Для этих столбов сделаем отдельные выборки с другими группировками и выполним JOIN
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбцов с самой популярной категорией товаров и id самого популярного мастера у покпателя. Для этих столбов сделаем отдельные выборки с другими группировками и выполним JOIN
                                T1.customer_id AS customer_id,
                                T1.customer_name AS customer_name,
                                T1.customer_address AS customer_address,
                                T1.customer_birthday AS customer_birthday,
                                ...
                                T1.report_period AS report_period
                                FROM (
                                    SELECT     -- в этой выборке достаём из DWH обновлённые или новые данные по покупателям, которые уже есть в витрине
                                            dcs.customer_id AS customer_id,
            								dcs.customer_name AS customer_name,
            								dcs.customer_address AS customer_address,
           									dcs.customer_birthday AS customer_birthday,
                                    ...
                                    (SELECT     
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id
                                                    ORDER BY count_product_for_craftsman_id DESC) AS T32 using(customer_id_for_top))                   
                AS T3 ON T2.customer_id = T3.customer_id_for_top 
                ) AS T4 WHERE T4.rank_count_product = 1 ORDER BY report_period -- условие позволяет оставить в выборке первую строку с наибольшим количеством товара одного типа купленным покупателем и с id мастера который продал наибольшее количество товаров покпателю
),
